...
| **M5** | 폴더 hover 지목 + YouTube IFrame 대기 | 02 | ✅ |
| **M6** | 폴더 해체 (찢기 1종 + 양손 동그라미 fallback) + 음악 본격 재생 | 03 | ✅ |
...
**M5 ✅** — Scene 02. 손 1초 hover로 폴더 지목, 파동·2배 확대·중앙 정렬, YouTube IFrame 로드 후 대기.

**M6 ✅** — Scene 03. 양손 wrist distance 변화로 *찢기* 검출(0.5s 윈도우, 0.20→0.45 임계), 8s 무반응 시 양손 동그라미 fallback. 검출되면 `lib/music-controller.ts`로 postMessage 명령(playVideo + unmute) → 첫 곡 재생, queue를 랜덤 셔플 후 Scene 04 (M7) 대기 메시지.

**M7 🔜** — Scene 04. 1장씩 분류 + 자라는 콜라주 (4 슬롯에 배치, "다음에" 5번째 제스처). plan 진입 시 결정사항 여기에 정리.

**잔여**: 튕기기·뭉개기 검출 + Scene 00 워밍업 자세 연습. 현 진척표 미배정 슬롯.
